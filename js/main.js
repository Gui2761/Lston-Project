...
// --- FAVORITOS (COM LIMPEZA AUTOMÁTICA) ---
...
function atualizarFavoritosUI() {
    ...
    if(favCount) {
        ...
        favCount.style.display = favoritos.length > 0 ? 'block' : 'none';
    }
    ...
    if(favoritos.length === 0) {
        lista.innerHTML = '<p style="text-align:center; padding:20px; color:#999;">Sua lista de desejos está vazia.</p>';
        return;
    }
        ...
        lista.innerHTML += `
            <div class="cart-item">
                <div style="display:flex;align-items:center;">
                    <img src="${img}" style="width:50px;height:50px;object-fit:cover;border-radius:4px;margin-right:10px;">
                    <div class="item-info">
                ...
                <div style="display:flex; gap:10px; align-items:center;">
                    <i class="fas fa-cart-plus" title="Mover para Carrinho" style="cursor:pointer; color:var(--green-color);" onclick="moverFavoritoParaCarrinho('${item.id}')"></i>
                    <i class="fas fa-trash" title="Remover" style="cursor:pointer; color:var(--danger-color);" onclick="window.toggleFavorito('${item.id}', this, event); atualizarFavoritosUI()"></i>
                </div>
            </div>`;
    });
}

window.moverFavoritoParaCarrinho = (id) => {
...
// --- LOJA (COM CORREÇÃO DE IDS FANTASMAS) ---
async function carregarLoja() {
    ...
    try {
        ...
        // --- CORREÇÃO: Limpeza de Favoritos Fantasmas ---
        // Verifica se os IDs salvos no navegador REALMENTE existem na lista de produtos
        const idsReais = todosProdutos.map(p => p.id);
        const favoritosValidos = favoritos.filter(id => idsReais.includes(id));

        // Se houver diferença, atualiza o localStorage e a variável
        if (favoritosValidos.length !== favoritos.length) {
            favoritos = favoritosValidos;
            localStorage.setItem('lston_favoritos', JSON.stringify(favoritos));
            console.log("Favoritos limpos automaticamente.");
        }
        // ------------------------------------------------

        if(container) exibirProdutos(todosProdutos);
        atualizarFavoritosUI(); // Atualiza o contador correto (agora pode ser 0)
...
// --- CARRINHO, CHECKOUT E FRETE (MANTIDOS) ---
...
function atualizarCarrinhoUI() {
    ...
    let texto = freteValor > 0 ? `Total: ${window.fmtMoney(total)} <br><small style="font-size:11px;">(c/ frete)</small>` : `Total: ${window.fmtMoney(total)}`;
    ...
    if(currentUserId) {
        try {
            ...
            if (docSnap.exists()) {
                ...
                if(dados.telefone) { document.getElementById('check-tel').value = dados.telefone; window.mascaraTel(document.getElementById('check-tel')); }
...
window.confirmarPedido = async () => {
    ...
    const confirmacao = await Swal.fire({ title: 'Confirmar Pedido?', html: `<div style="text-align:left; font-size:14px; color:#333;"><p><strong>Cliente:</strong> ${nome}</p><p><strong>Telefone:</strong> ${telefone}</p><p><strong>Endereço:</strong> ${endereco}</p><hr style="margin:10px 0;"><ul style="list-style:none; padding:0;">${resumoHtml}</ul><hr style="margin:10px 0;"><p style="text-align:right;">Total: <strong>${window.fmtMoney(totalFinal)}</strong></p></div>`, icon: 'question', showCancelButton: true, confirmButtonColor: '#8bc34a', cancelButtonColor: '#d33', confirmButtonText: '✅ Confirmar' });
    if (!confirmacao.isConfirmed) return;
    window.toggleLoading(true);
    try {
        for (const item of carrinho) { const prodSnap = await getDoc(doc(db, "produtos", item.id)); if (!prodSnap.exists()) throw new Error("Produto não existe mais."); if (parseInt(prodSnap.data().estoque) < item.qtd) throw new Error(`Estoque insuficiente: ${item.nome}`); }
        await addDoc(collection(db, "pedidos"), { cliente: nome, telefone, endereco, cidade, cep, pagamento, itens: carrinho, total: totalFinal, frete: freteValor, data: new Date().toISOString(), status: "Recebido", userEmail: currentUserEmail });
        for (const item of carrinho) { const ref = doc(db, "produtos", item.id); const snap = await getDoc(ref); const nv = (parseInt(snap.data().estoque) || 0) - item.qtd; await updateDoc(ref, { estoque: nv }); }
        await Swal.fire('Sucesso!', `Pedido realizado!`, 'success'); carrinho=[]; localStorage.setItem('lston_carrinho', '[]'); atualizarCarrinhoUI(); window.location.href="index.html";
    } catch(e) { window.toggleLoading(false); Swal.fire('Erro', e.message, 'error'); }
}
