...
// --- TABELA DE FRETE (ORIGEM: ARACAJU/SE) ---
const TABELA_FRETE = {
    // Local (Aracaju e Sergipe)
...
// NOVA: Máscara de Telefone (Bloqueia letras e formata)
...
// Aplica a máscara ao campo de telefone do checkout
const checkoutTelInput = document.getElementById('check-tel');
if(checkoutTelInput) {
    checkoutTelInput.addEventListener('input', function() { window.mascaraTel(this); });
}
...
// --- CHECKOUT INTELIGENTE (Auto-preenchimento) ---
window.irParaCheckout = async () => { 
    ...
    if(currentUserId) {
        try {
            ...
            if (docSnap.exists()) {
                ...
                if(dados.telefone) {
                    ...
                    window.mascaraTel(telField); // Aplica máscara visualmente
                }
        ...
        } catch(e) { console.log("Erro auto-preencher", e); }
    }
}
...
window.confirmarPedido = async () => {
    ...
    if(!nome || !endereco || !telefone) {
        return window.showToast("Preencha Nome, Endereço e Telefone!", "error");
    }

    // 2. VERIFICAÇÃO DE NÚMERO (TELEFONE)
    // Remove formatação para contar dígitos
    const telLimpo = telefone.replace(/\D/g, '');
    if (telLimpo.length < 10 || telLimpo.length > 11) {
        return window.showToast("Telefone inválido! Digite o DDD + Número.", "error");
    }
    
    // Calcula totais
    let subtotal = 0;
    carrinho.forEach(i => subtotal += parseFloat(i.preco) * i.qtd);
    const totalFinal = (subtotal - (subtotal * desconto)) + freteValor;

    // 3. MODAL DE CONFIRMAÇÃO (Resumo)
    ...
    const confirmacao = await Swal.fire({
        ...
        html: `
            <div style="text-align:left; font-size:14px; color:#333;">
                ...
                <p><strong>Endereço:</strong> ${endereco}<br><small>${cidade} - ${cep}</small></p>
                <p><strong>Pagamento:</strong> ${pagamento}</p>
                <hr style="margin:10px 0; border-top:1px solid #eee;">
                <ul style="list-style:none; padding:0;">${resumoHtml}</ul>
                <hr style="margin:10px 0; border-top:1px solid #eee;">
                <p style="text-align:right; font-size:16px;">Total com Frete: <strong>${window.fmtMoney(totalFinal)}</strong></p>
            </div>
        `,
        icon: 'question',
    ...
    try {
        // 4. VERIFICAÇÃO DE ESTOQUE NO SERVIDOR
        for (const item of carrinho) {
            ...
            const estoqueAtual = parseInt(prodSnap.data().estoque) || 0;
        ...
        await Swal.fire('Sucesso!', `Pedido realizado! Enviamos um resumo para seu e-mail.`, 'success');
        ...
        Swal.fire('Erro', e.message || "Erro ao processar pedido.", 'error');
    }
}

// --- (OUTRAS FUNÇÕES: FRETE, BUSCA, UI) ---
window.calcularFreteCarrinho = async () => {
    ...
    if (cep.length !== 8) { resultDiv.innerText = "CEP inválido."; resultDiv.style.color = "red"; return; }
    resultDiv.innerText = "Consultando...";
    window.toggleLoading(true);
    try {
        const res = await fetch(`https://viacep.com.br/ws/${cep}/json/`);
        const data = await res.json();
        if (data.erro) { resultDiv.innerText = "CEP não encontrado."; resultDiv.style.color = "red"; } 
        else {
            const uf = data.uf;
    ...
    } catch (e) { resultDiv.innerText = "Erro ao calcular."; } finally { window.toggleLoading(false); }
}

function calcularSimilaridade(s1, s2) { let longer = s1.length < s2.length ? s2 : s1; let shorter = s1.length < s2.length ? s1 : s2; if (longer.length === 0) return 1.0; return (longer.length - editDistance(longer, shorter)) / parseFloat(longer.length); }
...
async function carregarLoja() { if(container) container.innerHTML = '<p style="text-align:center;">Carregando...</p>'; carregarBanners(); try { const q = await getDocs(collection(db, "produtos")); todosProdutos = []; q.forEach((doc) => { todosProdutos.push({id:doc.id, ...doc.data()}); }); if(container) exibirProdutos(todosProdutos); } catch (e) { console.error(e); } }
function exibirProdutos(lista) { if(!container) return; container.innerHTML = ''; if(lista.length === 0) { container.innerHTML = '<p style="text-align:center; width:100%; padding:20px;">Nenhum produto encontrado.</p>'; return; } lista.forEach(prod => { const card = document.createElement('div'); card.className = 'product-card'; let capa = (prod.imagens && prod.imagens.length > 0) ? prod.imagens[0] : (prod.img || ''); const est = parseInt(prod.estoque) || 0; let priceHtml = `<div class="new-price">${window.fmtMoney(prod.preco)}</div>`; if(prod.precoOriginal && prod.precoOriginal > prod.preco) priceHtml = `<div class="old-price">${window.fmtMoney(prod.precoOriginal)}</div><div class="new-price">${window.fmtMoney(prod.preco)}</div>`; const heartClass = favoritos.includes(prod.id) ? 'fas fa-heart fav-btn active' : 'far fa-heart fav-btn'; card.innerHTML = `<i class="${heartClass}" onclick="toggleFavorito('${prod.id}', this, event)"></i><div class="product-img" style="background-image: url('${capa}'); cursor: pointer;" onclick="window.location.href='produto.html?id=${prod.id}'"></div><div style="width:100%;"><h3>${prod.nome}</h3><div class="price-box">${priceHtml}</div><div class="card-qty-selector"><button class="card-qty-btn" onclick="alterarQtdCard(this, -1)">-</button><input type="text" class="card-qty-input" value="1" readonly><button class="card-qty-btn" onclick="alterarQtdCard(this, 1)">+</button></div></div><button class="btn-add" onclick="adicionarComQtd('${prod.id}', this)" ${est===0?'disabled':''}>${est===0?'Esgotado':'Adicionar'}</button>`; container.appendChild(card); }); }
